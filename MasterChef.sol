...
contract MasterChef is Ownable {
    ...
    // The EGG TOKEN!
    FulcrumToken public fulcrum;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public fulcrumPerBlock;
    // Bonus muliplier for early fulcrum makers.
    ...
    constructor(
        FulcrumToken _fulcrum,
        address _devaddr,
        address _feeAddress,
        uint256 _fulcrumPerBlock,
        uint256 _startBlock
    ) public {
        fulcrum = _fulcrum;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        fulcrumPerBlock = _fulcrumPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 fulcrumReward = multiplier.mul(fulcrumPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(fulcrumReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 fulcrumReward = multiplier.mul(fulcrumPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        fulcrum.mint(devaddr, fulcrumReward.div(10));
        fulcrum.mint(address(this), fulcrumReward);
        pool.accEggPerShare = pool.accEggPerShare.add(fulcrumReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe fulcrum transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 fulcrumBal = fulcrum.balanceOf(address(this));
        if (_amount > fulcrumBal) {
            fulcrum.transfer(_to, fulcrumBal);
        } else {
            fulcrum.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _fulcrumPerBlock) public onlyOwner {
        massUpdatePools();
        fulcrumPerBlock = _fulcrumPerBlock;
    }
}
